...
cdef class PyFunctionReal(PyTransformable):
  def __cinit__(self, init = True):
    if not init:
      return
    if type(self)!=PyFunctionReal:
      return
    self.thisptr = <Transformable*> new Function[double]()
  ...
  def get_fn_values(self, component = None):
    ...
    r = []
    for i in range((<Function[double]*> self.thisptr).get_num_points()):
      r.append(cr[i])
    return r
  ...
  def get_dx_dy_values(self, int component):
    ...
    cdef int i
    if component is not None:
      (<Function[double]*> self.thisptr).get_dy_values(dx,dy,component)
    else:
      (<Function[double]*> self.thisptr).get_dy_values(dx,dy)
    rx = []
    ry = []
    for i in range((<Function[double]*> self.thisptr).get_num_points()):
      rx.append(dx[i])
