...
cdef class PyH1SpaceComplex(PySpaceComplex):
  def __cinit__(self,PyMesh mesh, boundary_conditions = None, p_init = None, shapeset = None, init = True):
    if not init:
      return
    if type(self)!=PyH1SpaceComplex:
      return
    if isinstance(p_init,PyShapeset):
      shapeset = p_init
      p_init = None
    ...
    if boundary_conditions is not None:
      if shapeset is not None:
        self.thisptr = <Space[cComplex[double]]*> new H1Space[cComplex[double]](mesh.thisptr, (<PyEssentialBCsComplex> boundary_conditions).thisptr, p_init, (<PyShapeset> shapeset).thisptr)
        return
      if p_init is not None:
        self.thisptr = <Space[cComplex[double]]*> new H1Space[cComplex[double]](mesh.thisptr, (<PyEssentialBCsComplex> boundary_conditions).thisptr, <int> p_init)
        return
      self.thisptr = <Space[cComplex[double]]*> new H1Space[cComplex[double]](mesh.thisptr, (<PyEssentialBCsComplex> boundary_conditions).thisptr)
    else:
      if shapeset is not None:
        self.thisptr = <Space[cComplex[double]]*> new H1Space[cComplex[double]](mesh.thisptr, <int> p_init, (<PyShapeset> shapeset).thisptr)
        return
      if p_init is not None:
        self.thisptr = <Space[cComplex[double]]*> new H1Space[cComplex[double]](mesh.thisptr, <int> p_init)
        return
      self.thisptr = <Space[cComplex[double]]*> new H1Space[cComplex[double]](mesh.thisptr)

  def fix_vertex(self, int id, value = None):
    if value is not None:
      (<H1Space[cComplex[double]]*> self.thisptr).fix_vertex(id, ccomplex(value))
    else:
      (<H1Space[cComplex[double]]*> self.thisptr).fix_vertex(id)
  def load(self, char *filename, PyMesh mesh, essential_bcs = None, shapeset = None):
    if isinstance(essential_bcs,PyShapeset):
      shapeset = essential_bcs
      essential_bcs = None
    if essential_bcs is not None:
      if shapeset is not None:
        (<H1Space[cComplex[double]]*> self.thisptr).load(filename, mesh.thisptr, (<PyEssentialBCsComplex> essential_bcs).thisptr, (<PyShapeset> shapeset).thisptr)
      else:
        (<H1Space[cComplex[double]]*> self.thisptr).load(filename, mesh.thisptr, (<PyEssentialBCsComplex> essential_bcs).thisptr)
    else:
      if shapeset is not None:
        (<H1Space[cComplex[double]]*> self.thisptr).load(filename, mesh.thisptr, (<PyShapeset> shapeset).thisptr)
      else:
        (<H1Space[cComplex[double]]*> self.thisptr).load(filename, mesh.thisptr)
